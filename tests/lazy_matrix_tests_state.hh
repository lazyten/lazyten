...
struct LazyMatrixSystemUnderTest {
   ...
   *              to in global scope.
   * */
  LazyMatrixSystemUnderTest(matrix_type mat,
  ...
  const stored_matrix_type& copy_to_internal_storage(const stored_matrix_type& m) {
    stored_matrices.push_back(m);
    return stored_matrices.back();
  }
...
struct AddMatrix : CommandBase<TestingTraits> {
  ...
  void show(std::ostream& os) const override { os << "AddMatrix (" << term << ")"; }
};  // AddMatrix
...
struct SubtractMatrix : CommandBase<TestingTraits> {
  ...
  void show(std::ostream& os) const override { os << "SubtractMatrix (" << term << ")"; }
};  // SubtractMatrix
...
struct MultiplyMatrix : CommandBase<TestingTraits> {
  ...
  MultiplyMatrix(const model_type& model)
        : term{*FixedSizeMatrix<MatrixTermType>::fixed_size(
                      model.n_cols(),
                      *gen::numeric_size<2>().as("No of columns of multiplied matrix"))
  ...
  void show(std::ostream& os) const override { os << "MultiplyMatrix (" << term << ")"; }
};  // MultiplyMatrix
...
struct StatefulTestingLibrary {
  ...
  void run_check(const model_type& initial_state, GenFunc&& generation_func,
    ...
    LazyMatrixWrapper<stored_matrix_type> wrap{std::move(model_type{initial_state})};
