...
struct RangeWithin {
  ...
  static Gen<krims::Range<value_type>> range_within(value_type min, value_type max,
                                                    value_type min_length = 0) {

    auto gen_range = [=] {
      if (min > max) {
        std::stringstream ss;
        ss << "Minimum " << min << " not smaller or equal to maximum " << max;
      ...
      auto start = *gen::scale(2.0, gen::inRange<value_type>(min, max - length + 1));
      if (start + length > max) {
        std::stringstream ss;
        ss << "Something went wrong: Too large maximum " << start + length
           << " where desired max was " << max;
        throw rc::GenerationFailure(ss.str());
      }
