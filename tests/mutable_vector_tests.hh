...
void ComparativeTests<Model, Sut>::run_all(
  ...
  CHECK(gen.run_test(prefix + "Altering elements via ()", test_setting_elements, eps));
  CHECK(gen.run_test(prefix + "Altering elements via []",
                     test_setting_elements_vectorised, eps));
  CHECK(gen.run_test(prefix + "Altering elements via iterator", test_readwrite_iterator,
                     eps));

  // Operations
  CHECK(gen.run_test(prefix + "Multiplication by scalar", test_multiply_scalar));
