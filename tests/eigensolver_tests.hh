...
class TestProblemRunner {
  public:
    ...
    void run_matching(
          std::function<bool(const EigensolverTestProblemBase<matrix_type>&)>
                predicate) const;

    /** Run all test problems which the library yields */
    void run_all() const {
        run_matching([](const EigensolverTestProblemBase<matrix_type>&) {
            return true;
        });
    }
    ...
    TestProblemRunner(std::function<typename testproblem_type::soln_type(
                            const testproblem_type&)>
                            solvefctn)
...
void TestProblemRunner<Testproblem>::run_matching(
      std::function<bool(const EigensolverTestProblemBase<matrix_type>&)>
            predicate) const {
    const std::vector<testproblem_type> problems =
          testproblemlib_type::get_all();
