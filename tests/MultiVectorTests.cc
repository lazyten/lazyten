...
std::vector<Vector> gen_vectors(size_t n_vecs) {
  ...
  return *gen::container<std::vector<Vector>>(n_vecs,
                                              gen::numeric_tensor<Vector>(n_elem));
}
...
TEST_CASE("MultiVector class", "[MultiVector]") {
  ...
  SECTION("Subview, deep and shallow copy") {
    auto test = []() {
      ...
      for (size_type i = 0; i < vecs.size(); ++i) {
        RC_ASSERT(view.at_ptr(i).is_shared_ptr() == mv.at_ptr(i).is_shared_ptr());
  ...
  SECTION("Obtaining memptrs") {
    auto test = []() {
      ...
      static_assert(
            std::is_same<decltype(*(cmv.memptrs()[0])), const scalar_type&>::value,
