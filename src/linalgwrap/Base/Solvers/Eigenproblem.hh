...
class EigenproblemBase {
  public:
    ...
    // matrix type:
    static_assert(std::is_same<stored_matrix_type,
                               typename StoredTypeOf<MatrixDiag>::type>::value,
    ...
    static_assert(IsMatrix<MatrixA>::value,
                  "MatrixA needs to be a matrix type");
    static_assert(IsMatrix<MatrixDiag>::value,
                  "MatrixDiag needs to be a matrix type");
    ...
    EigenproblemBase(const MatrixA& A, size_type n_ep, const MatrixDiag& diag)
        ...
        if (hermitian) {
            ...
            const real_type tolerance =
                  100 * Constants<real_type>::default_tolerance;
            assert_dbg(m_A_ptr->is_hermitian(tolerance),
                       ExcMatrixNotHermitian());
            assert_dbg(m_diag_ptr->is_hermitian(tolerance),
                       ExcMatrixNotHermitian());
        }
#endif
    }
...
class Eigenproblem : public EigenproblemBase<isHermitian, MatrixA, MatrixDiag> {
  public:
    ...
    static_assert(
          std::is_same<stored_matrix_type,
                       typename StoredTypeOf<MatrixB>::type>::value,
          "For a general eigenproblem the MatrixB type and the MatrixA type "
          "need to have the same underlying stored matrix type.");
    static_assert(IsMatrix<MatrixB>::value,
                  "MatrixB needs to be a matrix type");
     ...
     */
    Eigenproblem(const MatrixA& A, const MatrixB& B, size_type n_ep,
                 const MatrixDiag& Diag)
    ...
    Eigenproblem(
          const typename std::enable_if<
                std::is_same<MatrixA, MatrixDiag>::value, MatrixA>::type& A,
...
struct Eigenproblem<isHermitian, MatrixA, void, MatrixDiag>
    ...
    const MatrixA B() const {
        assert_dbg(false,
                   krims::ExcDisabled("The method B() for a non-generalised "
