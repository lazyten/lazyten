...
DefSolverException0(ExcArmadilloEigensolverFailed,
                    << "Armadillo eigensolver failed");
 ...
 */
template <typename Eigenproblem,
          typename State = ArmadilloEigensolverState<Eigenproblem>>
class ArmadilloEigensolver : public EigensolverBase<State> {
    ...
    static_assert(
          std::is_same<
                typename Eigenproblem::stored_matrix_type,
  ...
  public:
    ...
    ArmadilloEigensolver(const krims::ParameterMap& map)
          : ArmadilloEigensolver() {
  ...
  private:
    struct eigenpairsorter {
        template <typename T>
        bool operator()(typename std::enable_if<
                              krims::IsComplexNumber<T>::value, T>::type i,
                        T j) const {
    ...
    std::vector<size_t> argsort_by_real_part(
...
std::vector<size_t>
ArmadilloEigensolver<Eigenproblem, State>::argsort_by_real_part(
      const arma::Col<evalue_type>& eval_arma) const {

    if (!Eigenproblem::generalised && Eigenproblem::hermitian &&
        Eigenproblem::real) {
        ...
        return krims::argsort(
              std::begin(eval_arma), std::end(eval_arma),
...
void ArmadilloEigensolver<Eigenproblem, State>::copy_to_solution(
    ...
    if ((Eigenproblem::real && base_type::which[1] == 'R') ||
        ...
        idcs = std::vector<size_t>(std::begin(idcs) + start,
                                   std::begin(idcs) + end);
    } else {
...
void ArmadilloEigensolver<Eigenproblem, State>::assert_valid_control_params(
    ...
    const bool complexok = krims::IsComplexNumber<evalue_type>::value &&
                           (which == "SI" || which == "LI");
    solver_assert(which == "SM" || which == "LM" || which == "LR" ||
                        which == "SR" || complexok,
...
template <typename Eigenproblem, typename State>
void ArmadilloEigensolver<Eigenproblem, State>::solve_state(
      state_type& state) const {
    ...
    const bool res = detail::ArmadilloEigWrapper<Eigenproblem>::eig(
          problem, eval_arma, evec_arma);
