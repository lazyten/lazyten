...
template <typename StoredMatrix>
void LazyMatrix_i<StoredMatrix>::extract_block(stored_matrix_type& M,
        ...
        for (size_type col = 0; col < M.n_cols(); ++col) {
            switch (mode) {
                case Transposed::None:
                    M(row, col) =
                          c_this * (*this)(start_row + row, start_col + col) +
                          c_M * M(row, col);
                    break;
                case Transposed::Trans:
                    M(row, col) =
                          c_this * (*this)(start_col + col, start_row + row) +
                          c_M * M(row, col);
                    break;
                case Transposed::ConjTrans:
                    ...
                    M(row, col) = c_this * mconj((*this)(start_col + col,
                                                         start_row + row)) +
                                  c_M * M(row, col);
                    break;
            }  // mode
...
void LazyMatrix_i<StoredMatrix>::apply(
    ...
    const size_type size =
          mode == Transposed::None ? this->n_rows() : this->n_cols();
            ...
            for (size_type col = 0; col < in.n_cols(); ++col) {
                switch (mode) {
                    ...
                    case Transposed::ConjTrans:
                        ...
                        detail::ConjFctr mconj;
                        out(row, col) +=
                              c_this * mconj((*this)(k, row)) * in(k, col);
                        break;
                }  // mode
            }      // col
        }          // row
