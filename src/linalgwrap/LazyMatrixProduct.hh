...
class LazyMatrixProduct : public LazyMatrixExpression<StoredMatrix> {
  public:
    ...
    bool has_transpose_operation_mode() const override {
        ...
        return std::all_of(std::begin(m_factors), std::end(m_factors),
                           [](const factor_ptr_type& p) {
                               return p->has_transpose_operation_mode();
                           });
    }
...
void LazyMatrixProduct<StoredMatrix>::extract_block(
    ...
    if (mode == Transposed::None) {
        ...
        extract_block_inner(m_factors.rbegin(), m_factors.rend(), M, start_row,
                            start_col, mode, c_this, c_M);
    } else {
        // Here we exploit that
        // (A0 A1 A2 ... An)^T M = An^T ... A2^T A1^T A0^T M
        extract_block_inner(m_factors.begin(), m_factors.end(), M, start_row,
                            start_col, mode, c_this, c_M);
    }
}
...
void LazyMatrixProduct<StoredMatrix>::extract_block_inner(
      ...
      const size_type start_row, const size_type start_col,
      const Transposed mode, const scalar_type c_this,
      const scalar_type c_M) const {
    ...
    const size_type cols =
          mode == Transposed::None ? (*last)->n_cols() : (*last)->n_rows();
...
void LazyMatrixProduct<StoredMatrix>::apply(
    ...
    if (mode == Transposed::None) {
        ...
        apply_inner(m_factors.rbegin(), m_factors.rend(), x, y, mode, c_this,
                    c_y);
    } else {
        // Here we exploit that
        // (A0 A1 A2 ... An)^T M = An^T ... A2^T A1^T A0^T M
        apply_inner(m_factors.begin(), m_factors.end(), x, y, mode, c_this,
                    c_y);
    }
}
...
void LazyMatrixProduct<StoredMatrix>::apply_inner(
    ...
    if (mode == Transposed::None) {
        assert_dbg(tmp.n_elem() == (*last)->n_cols(),
                   krims::ExcInternalError());
    } else {
        assert_dbg(tmp.n_elem() == (*last)->n_rows(),
                   krims::ExcInternalError());
    }
...
void LazyMatrixProduct<StoredMatrix>::mmult(const stored_matrix_type& in,
    ...
    if (mode == Transposed::None) {
        // Go about factors in reverse order (i.e. applying from right to left
        // to the supplied input recursively)
        mmult_inner(m_factors.rbegin(), m_factors.rend(), in, out, mode, c_this,
                    c_out);
    } else {
        // Here we exploit that
        // (A0 A1 A2 ... An)^T M = An^T ... A2^T A1^T A0^T M
        mmult_inner(m_factors.begin(), m_factors.end(), in, out, mode, c_this,
                    c_out);
    }
}
...
void LazyMatrixProduct<StoredMatrix>::mmult_inner(
    ...
    const auto last = end - 1;
    if (mode == Transposed::None) {
        assert_dbg(tmp.n_rows() == (*last)->n_cols(),
                   krims::ExcInternalError());
    } else {
        assert_dbg(tmp.n_rows() == (*last)->n_rows(),
                   krims::ExcInternalError());
    }
...
void LazyMatrixProduct<StoredMatrix>::multiply_in_place(BidirectIterator begin,
                                                        ...
                                                        Transposed mode) {
    for (; begin != end; ++begin) {
        const size_type rows = mode == Transposed::None ? (*begin)->n_rows()
                                                        : (*begin)->n_cols();
        stored_matrix_type tmp(rows, m.n_cols(), false);
...
template <typename StoredMatrix>
template <typename BidirectIterator>
void LazyMatrixProduct<StoredMatrix>::apply_in_place(
      BidirectIterator begin, BidirectIterator end,
      MultiVector<vector_type>& mv, Transposed mode) {
    for (; begin != end; ++begin) {
        const size_type rows = mode == Transposed::None ? (*begin)->n_rows()
