...
//
// Perform iterations:
extern "C" void dsaupd_(int* ido, char* bmat, const unsigned int* n,
...
extern "C" void dseupd_(int* rvec, char* howmany, int* select, double* d,
                        ...
                        const unsigned int* n, char* which,
                        const unsigned int* nev, const double* tol,
                        double* resid, int* ncv, double* v, int* ldv,
                        int* iparam, int* ipntr, double* workd, double* workl,
                        int* lworkl, int* info);
...
//
// Perform iterations:
extern "C" void dnaupd_(int* ido, char* bmat, const unsigned int* n,
                        char* which, const unsigned int* nev, const double* tol,
                        double* resid, int* ncv, double* v, int* ldv,
                        int* iparam, int* ipntr, double* workd, double* workl,
                        int* lworkl, int* info);
...
extern "C" void dneupd_(int* rvec, char* howmany, int* select, double* dr,
...
class ds_upd_wrapper {
  public:
    ...
    void arnoldi_step(int& ido, std::array<int, 11>& iparam, int& info) {
        dsaupd_(&ido, bmat.data(), &n, which.data(), &nev, &tol,
                resid_ptr->data(), &ncv, v_ptr->data(), &ldv, iparam.data(),
                ipntr.data(), workd_ptr->data(), workl_ptr->data(), &lworkl,
                &info);
    }
     ...
     */
    void eigenpairs(double sigma, std::array<int, 11>& iparam,
                    std::vector<double>& evalues, std::vector<double>& evectors,
                    int& info, bool compute_eigenvectors = true) {
        ...
        dseupd_(&rvec, &howmany, select.data(), evalues.data(), evectors.data(),
                &ldz, &sigma, bmat.data(), &n, which.data(), &nev, &tol,
                resid_ptr->data(), &ncv, v_ptr->data(), &ldv, iparam.data(),
                ipntr.data(), workd_ptr->data(), workl_ptr->data(), &lworkl,
                &info);
    }
     ...
     */
    ds_upd_wrapper(bool generalised_, size_t dim_, std::string which_,
                   size_t n_eigenpairs_, scalar_type tolerance_,
                   size_t n_arnoldi_vectors_,
