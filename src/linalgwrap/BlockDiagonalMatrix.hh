...
class BlockDiagonalMatrix : public BlockDiagonalMatrixBase<Matrices...> {
 public:
  ...
  explicit BlockDiagonalMatrix(Matrices&&... blocks)
        : BlockDiagonalMatrix{
                std::tuple<Matrices&&...>{std::forward<Matrices>(blocks)...}} {}
...
BlockDiagonalMatrix<typename std::decay<Matrices>::type...> make_block_diagonal(
  ...
  return BlockDiagonalMatrix<typename std::decay<Matrices>::type...>{std::move(blocks)};
}
...
template <typename... Matrices>
BlockDiagonalMatrix<Matrices...>::BlockDiagonalMatrix(std::tuple<Matrices&&...> blocks)
...
template <typename... Matrices>
typename BlockDiagonalMatrix<Matrices...>::scalar_type BlockDiagonalMatrix<Matrices...>::
operator()(size_type row, size_type col) const {
  ...
  assert_dbg(offset_blockptr != m_start_index_to_block.end(), krims::ExcInternalError());
