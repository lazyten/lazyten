...
class MultiVectorReadonly : public krims::Subscribable {
  ...
  public:
    ...
    typedef krims::DereferenceIterator<
          typename std::vector<vector_rcptr_type>::iterator>
          iterator;
    typedef krims::DereferenceIterator<
    ...
    vector_type& front() {
        assert_valid_state();
        assert_dbg(!empty(), krims::ExcInvalidState(
                                   "The MultiVector needs to be non-empty"));
        return *m_vs.front();
    }
    ...
    std::vector<const scalar_type*> memptrs(
          krims::enable_if_cond_same_t<IsMutableMemoryVector<Vector>::value,
                                       Vector, InnerVector>* dummy = 0) const;
    ///@}
...
template <typename InnerVector>
krims::RCPWrapper<InnerVector> MultiVectorReadonly<InnerVector>::at_ptr(
      size_type i) {
