...
#pragma once
#include "lazyten/MatrixIteratorCore.hh"
...
class MatrixIterator : public IteratorCore {
 public:
  ...
  static_assert(
        std::is_base_of<detail::MatrixIteratorCoreBase<original_matrix_type,
...
template <typename IteratorCore>
MatrixIterator<IteratorCore>::MatrixIterator(matrix_type& mat) : base_type{mat} {}
...
template <typename IteratorCore>
typename MatrixIterator<IteratorCore>::size_type MatrixIterator<IteratorCore>::col()
      const {
  return base_type::col();
}

template <typename IteratorCore>
typename MatrixIterator<IteratorCore>::index_type MatrixIterator<IteratorCore>::indices()
      const {
  return base_type::indices();
}

template <typename IteratorCore>
MatrixIterator<IteratorCore>& MatrixIterator<IteratorCore>::seek_to(index_type element) {
...
template <typename IteratorCore>
auto MatrixIterator<IteratorCore>::operator*() const -> decltype(this->value()) {
  base_type::assert_valid_state();
  return base_type::value();
}

template <typename IteratorCore>
auto MatrixIterator<IteratorCore>::operator-> () const -> decltype(this->ptr_to_value()) {
  base_type::assert_valid_state();
  return base_type::ptr_to_value();
}

template <typename IteratorCore>
bool MatrixIterator<IteratorCore>::operator==(const MatrixIterator& other) const {
