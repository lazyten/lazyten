...
class MatrixPrinter {
 ...
 private:
   ...
   * */
  template <typename Scalar>
  std::streamsize determine_width(const Matrix_i<Scalar>& mat, std::ostream& out) const;
   ...
   * */
  template <typename Scalar>
  void print_elem(const Scalar& elem, std::streamsize width, std::ostream& out) const;

  /** Print a zero, either one which is or is not explicitly stored
   *
   * \param stored   Is the zero explicitly stored?
   * */
  void print_zero(bool stored, std::streamsize width, std::ostream& out) const;
...
inline MatrixPrinter::MatrixPrinter() : m_max_screen_width(80), m_known_zero_char('*') {}

inline void MatrixPrinter::max_screen_width(std::streamsize w) { m_max_screen_width = w; }

inline std::streamsize MatrixPrinter::max_screen_width() const {
  return m_max_screen_width;
}

inline void MatrixPrinter::known_zero_character(char c) { m_known_zero_char = c; }

inline char MatrixPrinter::known_zero_character() const { return m_known_zero_char; }

template <typename Scalar>
void MatrixPrinter::print(const Matrix_i<Scalar>& mat, std::ostream& out) const {
