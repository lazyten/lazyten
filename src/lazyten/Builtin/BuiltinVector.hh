...
class BuiltinVector : public PtrVector<Scalar>, public Stored_i {
 public:
  ...
  BuiltinVector(std::initializer_list<Scalar> list) : BuiltinVector(list.size(), false) {
    std::move(std::begin(list), std::end(list), base_type::begin());
  }

  /** \brief Construct from std::vector */
  explicit BuiltinVector(std::vector<scalar_type> v) : BuiltinVector(v.size(), false) {
  ...
  /** Copy constructor */
  BuiltinVector(const BuiltinVector& other) : BuiltinVector(other.m_size, false) {
    std::copy(other.begin(), other.end(), base_type::begin());
  }
